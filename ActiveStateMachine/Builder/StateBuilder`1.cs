...
public sealed class StateBuilder<TTrigger>
{
    ...
    internal List<TransitionSpec<TTrigger>> Transitions { get; } = [];

    internal StateBuilder(string stateName) => StateName = stateName;

    public StateBuilder<TTrigger> AsTerminal()
    {
        IsTerminal = true;
        return this;
    }

    public StateBuilder<TTrigger> OnEnter(string name, Action action)
    {
        EntryActions.Add(new TransitionAction(name, action));
        return this;
    }

    public StateBuilder<TTrigger> OnExit(string name, Action action)
    {
        ExitActions.Add(new TransitionAction(name, action));
        return this;
    }

    public TransitionBuilder<TTrigger> On(TTrigger trigger) => new(this, trigger);
}